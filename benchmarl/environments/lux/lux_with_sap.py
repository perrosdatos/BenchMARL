...
class LuxSapTorchRLEnv(EnvBase):
    ...
    def _make_specs(self):
        ...
        agents_obs_spec = Composite({
            ...
            "action_mask": Bounded(
                low=0, high=1,
                shape=torch.Size([*self.batch_size, self.max_units, 6]),
                dtype=torch.bool,
                device=self.device
            )
        }, shape=torch.Size([*self.batch_size, self.max_units]))
    ...
    def _reset(self, tensordict: Optional[TensorDict] = None, **kwargs) -> TensorDict:
        ...
        if not hasattr(self, "env_state"):
            ...
            self.prev_full_mask = np.zeros((self.batch_size[0], 2, self.max_units), dtype=bool)
        ...
        if not hasattr(self, "opp_agents"):
            ...
            try:
                # In BenchMARL, Agent defaults to rulebased agent.
        ...
        if len(reset_indices) > 0:
            ...
            for idx, b_idx in enumerate(reset_indices):
                ...
                self.prev_full_mask[b_idx] = False
        ...
        action_mask = np.zeros((b_size, self.max_units, 6), dtype=np.bool_)
    ...
    def _step(self, tensordict: TensorDict) -> TensorDict:
        ...
        # Build action array (B, 16, 3), containing (action_id, dx, dy)
        action_3d = np.zeros((b_size, self.max_units, 3), dtype=np.int32)
        sap_mask = actions == 5
        action_3d[..., 0] = actions
        action_3d[sap_mask, 1] = 0
        action_3d[sap_mask, 2] = -1
        ...
        for b in range(b_size):
            ...
            if hasattr(self, "opp_actions") and self.opp_actions is not None:
                ...
                opp_3d = np.concatenate([self.opp_actions[b, ..., None], np.zeros((self.max_units, 2), dtype=np.int32)], axis=-1)
                if t_ids_np[b] == 0:
        ...
        # Extracted directly from Ground Truth (env_state) to bypass Fog of War for reward tracking
        m = np.asarray(self.env_state.units_mask, bool)
        u_pos_np = np.asarray(self.env_state.units.position, np.int32)
        ...
        if not hasattr(self, "prev_full_mask"):
            self.prev_full_mask = np.zeros((b_size, 2, self.max_units), dtype=bool)
            
        delta_masks = m.astype(np.int16) - self.prev_full_mask.astype(np.int16)
        
        # Convert absolute JAX arrays to Ego-centric arrays for combat logic (0 = My Team, 1 = Opponent)
        ego_delta_masks = np.zeros_like(delta_masks)
        ego_all_positions = np.zeros_like(u_pos_np) if u_pos_np is not None else None
        
        for b in range(b_size):
            t = t_ids_np[b]
            ego_delta_masks[b, 0] = delta_masks[b, t]
            ego_delta_masks[b, 1] = delta_masks[b, 1 - t]
            if u_pos_np is not None:
                ego_all_positions[b, 0] = u_pos_np[b, t]
                ego_all_positions[b, 1] = u_pos_np[b, 1 - t]
        ...
        shaped_global, shaped_local, reward_components = compute_combat_rewards(
            ...
            delta_masks=ego_delta_masks, # EGO-CENTRIC FIX
            all_positions=ego_all_positions # EGO-CENTRIC FIX
        )
        for b in range(b_size):
